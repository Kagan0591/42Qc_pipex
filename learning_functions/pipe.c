...
// Pipe exercise 1, simple read from the output of pipe
int	basic_pipe()
{
	int	fd[2];
	int	id;
	char *str = {"Bonjour! from the child to the parent\n"};
	char *str_from_child;
	...
	else
	{
		wait(NULL);
		close(fd[1]);
		if (read(fd[0], &str_from_child, sizeof(str)) == -1)
		{
			close(fd[0]);
			return (4);
		}
		close(fd[0]);
		printf("Return value str from child = %s\n", str_from_child);
	}
	return (0);
}
